...
def validate_research_dataset(bars: pd.DataFrame, spec: DatasetSpec | None = None) -> None:
    ...
    if spec.require_contiguous:
        ...
        if not (diffs == expected).all():
            raise ValueError(f"dataset is not contiguous at expected frequency {spec.expected_frequency}")

def normalize_research_dataset(bars: pd.DataFrame, spec: DatasetSpec | None = None) -> pd.DataFrame:
